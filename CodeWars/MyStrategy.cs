...
/**
 * TODO:
 * - неправильна€ сумма полей
 * - лечение в ѕѕ
 * - пр€тать вертолеты от самолетов
 * - оптимизировать начальное построение
 * 
 * - €дерка: перебрать несколько центров, учитывать уклонени€
 */
                ...
                Func<AMove, double> checkAction = move =>
                {
                    ...
                    if (partialEnv != null)
                    {
                        foreach (var veh in env.Vehicles)
                        {
                            ...
                            if (veh.RemainingAttackCooldownTicks > 0) // у тех, кто стрел€л давно, откатываем кд
