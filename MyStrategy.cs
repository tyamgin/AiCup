...
namespace Com.CodeGame.CodeTroopers2013.DevKit.CSharpCgdk
{
    public partial class MyStrategy : IStrategy
    {
        public void Move(Trooper self, World world, Game game, Move move)
        {
            ...
            // может возникнуть такое, что очков на выстрел нету, и он попытаетс€ пойти на клетку врага
            Point ifShot = IfShot();
            if (ifShot != null)
            {
                ...
                )
            {
                ...
                if (bestTurn.profit < Inf)
                {
                    ...
                    {   
                        // Ћибо двигатьс€ в окружение, чтобы только не загородить(увеличить кратчайший путь до бонуса)
