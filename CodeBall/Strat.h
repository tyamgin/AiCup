...
public:
    ...
    void act(AAction &action) {
        ...
        std::function<bool(AAction&, int&, int, int, int, int)> catchGkStrat4 = [&](AAction& resAction, int& resTicks, int drawWt, int drawI, int drawJ, int drawK) {
            if (env.roundTick < 50)
                return false;
            ...
            if (maxBallVel >= 0) {
                ...
                if (drawI < 0) {
                    AAction t1;
                    int t2;
                    catchGkStrat4(t1, t2, selWt, selI, selJ, selK);
                }
                prevI = selI;
                prevWt = selWt;
                return true;
