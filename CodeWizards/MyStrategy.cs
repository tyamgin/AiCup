 ...
 * - ”лучшить cancastmagicmissile
 * - когда мало жизней от фаербольшика держатьс€ подальше
 * - хаста, не изусать advanced mm
 * - не идти за нейтралами
 * - учитывать что бонусов скорее всего нет (или кто-то р€дом ходит со статусом)
                ...
                if (opp is AWizard)
                {
                    if (my.CastRange <= opp.CastRange)
                    {
                        if (GoAwayCond(my, opp as AWizard))
                            ...
                            my.CastRange = my.CastRange + 25; // HACK: чтобы не бо€тьс€ подходить к тем у кого прокачан range 
                    }
                }
                if (bld != null)
                {
